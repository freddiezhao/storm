...
public class RptRealtimeMontrCanclBolt extends SoHandler implements IRichBolt {

	private static final long serialVersionUID = 6886719014974925648L;
	private static final Logger logger = LoggerFactory
			.getLogger(RptRealtimeMontrCanclBolt.class);
	...
	public Object handleSoItem(Tuple tuple, JumpMQOrderVo parentOrder,
		...
		String updt_time = DateUtil.transferDateToString(order.getUpdateTime(),
				DateUtil.YYYY_MM_DD_HH_MM_SS_STR);
		Long whse_id = order.getWarehouseId();
		Long end_user_id = order.getEndUserId();
		Long prod_id = soItem.getProductId();

		String categ_lvl1_id = Get.getCtgIDorBrandIDFromProd(dao, prod_id
				.toString(), "categ_lvl1_id");
		String categ_lvl2_id = Get.getCtgIDorBrandIDFromProd(dao, prod_id
				.toString(), "categ_lvl2_id");
		String mg_brand_id = Get.getCtgIDorBrandIDFromProd(dao, prod_id
				.toString(), "mg_brand_id");
		String biz_unit = Get.getBizUnitByMrchtId(dao, soItem.getMerchantId()
				+ "");// 根据MerchntId获得biz_unit
		...
		try {
			...
			logger.info("INSERT FAILED " + soItem.baseInfo() + ". Caused by: "
					+ e.getMessage());
	...
	public void insertOracle(Connection con, Map<String, String> map) {
		{
			...
			try {
				String sql = "insert into "
						+ Constant.TABLE_RPT_REALTIME_MONTR_CANCL_S
						...
						+ "','yyyy-mm-dd')," + "to_date('"
						+ map.get("date_time_id")
						+ "','yyyy-mm-dd hh24:mi:ss')," + map.get("city_id")
						+ "," + map.get("ordr_srce") + ","
						+ map.get("sale_biz_type") + "," + map.get("whse_id")
						+ "," + map.get("biz_unit") + ","
						+ map.get("categ_lvl1_id") + ","
						+ map.get("categ_lvl2_id") + ","
						+ map.get("mg_brand_id") + "," + map.get("prod_id")
						+ "," + map.get("end_user_id") + ","
						+ map.get("new_cust_flag") + ","
				...
				try {
					...
					logger.info("error: insert  "+ Constant.TABLE_RPT_REALTIME_MONTR_CANCL_S+ " error ! ");
				}
