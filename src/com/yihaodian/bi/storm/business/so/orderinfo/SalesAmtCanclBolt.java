...
/**
 * 最终结果表到 
 * rpt.rpt_realtime_mp_categ_amt_c_s
 * rpt.rpt_realtime_mp_amt_cancl_s
 * @author lining2
 */
public class SalesAmtCanclBolt implements IRichBolt {

	private static final long serialVersionUID = 1L;
	
	private static final Logger logger = LoggerFactory.getLogger(SalesAmtCanclBolt.class);
	...
	public void insertData(Map<String, Double> regnAmountMap, GosOrderDao dao){
		...
		while(iterator.hasNext()) {
			...
			try {
				dao.insertRecord("rpt_realtime_mp_amt_cancl_test", key, "cf", "order_amount", CommonUtil.getFmatDouble(regnAmountMap.get(key)) + "");
			} catch (Exception e) {
				logger.error("insert hbase error. `rpt_realtime_mp_amt_cancl_test`.");
				e.printStackTrace();
			}
		}
	}
	public Map<String, Double> initCountMap(String rowKey, GosOrderDao dao) throws Exception{
		...
		List<Result> rsList = dao.getRecordByRowKeyRegex("rpt_realtime_mp_amt_cancl_test", rowKey) ;
	...
	public void insertOracleOrder(Connection con,Map<String, Double> countMap)
	{
		...
		try {
			...
			while(iterator.hasNext()) {
				...
				String sql 
				  = "insert into rpt.rpt_realtime_mp_amt_cancl_s(DATE_TIME_ID, BIZ_UNIT, PLTFM_LVL1_ID, PRMTN_RULE_TYPE, ORDR_AMT) " +
				  		"values (to_date('"+date_time_id+"','yyyy-MM-dd hh24:mi:ss'),'"+biz_unit+"','" + pltfm_lvl1_id + "','" + "-99" + "','" + countMap.get(key)+"')";
				//logger.info(sql);
				try {
					statement.execute(sql);
				} catch (SQLException e) {
					logger.info("insert rpt.rpt_realtime_mp_amt_cancl_s failed ! " + e.getMessage());
				}
			}
		} catch (Exception e){
			logger.error("insert rpt.rpt_realtime_mp_amt_cancl_s error !" + e.getMessage());
		}finally {
			try {
				statement.close() ;
			} catch (SQLException e) {
	...
	public void insertOracleOrderInCateg(Connection con, Map<String, Double> countCtgMap)
	{
		...
		try {
			...
			while(itor.hasNext()) {
				...
				String sql 
				  = "insert into rpt.rpt_realtime_mp_categ_amt_c_s(DATE_TIME_ID, BIZ_UNIT, PLTFM_LVL1_ID, PRMTN_RULE_TYPE,CATEG_LVL1_ID, ORDR_AMT) " +
				  		"values (to_date('"+date_time_id+"','yyyy-MM-dd hh24:mi:ss'),'"+biz_unit+"','" + pltfm_lvl1_id + "','" + "-99" + "','" + ctgid + "','" + countCtgMap.get(key)+"')";
				//logger.info(sql);
				try {
					statement.execute(sql);
				} catch (SQLException e) {
					logger.info("insert rpt.rpt_realtime_mp_categ_amt_c_s failed ! " + e.getMessage());
			    }	
			} 
		} catch (Exception e){
			logger.error("insert rpt.rpt_realtime_mp_categ_amt_c_s error !" + e.getMessage());
		} finally {
			try {
				statement.close() ;
			} catch (SQLException e) {
