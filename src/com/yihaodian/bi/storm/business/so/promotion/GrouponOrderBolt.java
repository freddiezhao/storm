 ...
 *
 */
public class GrouponOrderBolt implements IRichBolt {

	private static final long serialVersionUID = 3863543754168851281L;
	private static final Logger logger = LoggerFactory.getLogger(GrouponOrderBolt.class);
	...
	public void execute(Tuple input) {
		...
		try {
			...
			if (isDetailGrpon(orderDate, uid, prodID)) { //查看该soItem是否为团购小口径
	...
	//			String biz_unit = CommonOper.getBizUnitByMrchtId(dao, item.getMerchantId()+"");  //temporarily not used
				...
				String keyCtg = orderDate + "_grpon_ctg_detail_"  + ctgId; //20140709_grpon_ctg_detail_-13
				...
				//团购日累计总金额
				String key = orderDate + "_grpon_all_detail";
			...
			String ctgId = Get.getCtgIDFromProd(dao, soItemVo.getProductId().toString());
			if (ctgId == null)  ctgId = "-999999"; 
			String keyCtg = orderDate + "_grpon_ctg_broad_"  + ctgId; //20140709_grpon_ctg_broad_-13
			Double eachAmt = countCtgMap.get(keyCtg);
			if (eachAmt == null) {
				eachAmt = 0.0;
			}
			...
			//团购日累计总金额
			String key = orderDate + "_grpon_all_broad";
			Double amtDouble = countMap.get(key);//20140709_grpon_all_broad
			if (amtDouble == null) {
				amtDouble = 0.0;
			}
			...
			pl("error.GrouponOrderBolt:"+e1.getMessage());
		}
	...
	public void prepare(Map stormConf, TopologyContext context,
		...
		try {
			...
			countMap = initCountMap(Constant.TABLE_GROUPON_MP_AMT, currDate + "_grpon_all", dao) ;
			countCtgMap = initCountMap(Constant.TABLE_GROUPON_MP_AMT, currDate + "_grpon_ctg", dao) ;
			...
			while(iterator.hasNext()) {
				...
				String sql 
				  = "insert into edw1_dev.rpt_grpon_mp_amt(DATE_TIME_ID, PRMTN_RULE_TYPE, ORDR_AMT, DETL_FLAG) " +
				  		"values (to_date('"+date_time_id+"','yyyy-MM-dd hh24:mi')" + ",'2','" + countMap.get(key)+"','" + detailFlag + "')";
//				logger.info(sql);
				try {	
			...
			while(itor.hasNext()) {
				...
				String sql 
				  = "insert into edw1_dev.rpt_grpon_mp_categ_amt(DATE_TIME_ID, PRMTN_RULE_TYPE, CATEG_LVL1_ID, ORDR_AMT, DETL_FLAG) " +
				  		"values (to_date('"+date_time_id+"','yyyy-MM-dd hh24:mi')" + ",'2','" + ctgid + "','" + countCtgMap.get(key)+"','" + detailFlag + "')";
//				logger.info(sql);
				try {	
					statement.execute(sql);
				} catch (Exception e) {
	...
	private static boolean isDetailGrpon(String date, String uid, String prodID) throws IOException{
		...
		if (dao.exists(Constant.TABLE_GROUPON_USER, today + "_" + uid +  "_" + prodID) ||
				dao.exists(Constant.TABLE_GROUPON_USER, yesterday + "_" + uid + "_" + prodID) ||
				dao.exists(Constant.TABLE_GROUPON_USER, theDayBefore + "_" + uid + "_" + prodID))
			return true;
